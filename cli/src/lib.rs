...
fn stream_logs(config: &WithPath<Config>, rpc_url: &str, is_first_suite: bool) -> Result<Vec<std::process::Child>> {
    ...
    if let Some(test) = config.test_validator.as_ref() {
        if let Some(genesis) = &test.genesis {
            for entry in genesis {
                let log_file = File::create(format!("{}/{}-{}.log", program_logs_dir, entry.address, file_index))?;
                let stdio = std::process::Stdio::from(log_file);
                let child = std::process::Command::new("solana")
                    .arg("logs")
